...
3. ผู้ใช้แก้ไขข้อมูล (น้ำหนัก ส่วนสูง เป้าหมาย รูปแบบ IF ที่ต้องการ)
4. กดบันทึก → ระบบอัปเดตข้อมูลผ่าน Supabase และคำนวณค่าที่เกี่ยวข้องใหม่ (เช่น BMI)
...
3. แสดงผลเป็นกราฟและการ์ดสรุป (Chart/Widget) บนหน้า Dashboard
...
- ฟิลด์: user_id (UUID, FK → users.user_id), weight (number), height (number), goal (text), if_pattern (text เช่น 16:8)
...
3. **หน้า Health Dashboard** - แสดงกราฟและสถิติข้อมูลสุขภาพรายสัปดาห์/รายเดือน รวมถึงข้อมูลจาก Google Health API
4. **หน้า Healthy Journey** - แสดงภารกิจ คะแนนสะสม ระดับ และ badges
5. **หน้า Profile** - แสดงข้อมูลผู้ใช้ การตั้งค่า IF การเชื่อมต่อ Google Health และการตั้งค่าการแจ้งเตือน
